# jcamd.com

Personal portfolio for John Cameron Davis — builder of AI products in Austin,
Texas. Static HTML/CSS, no build step, served by the Vercel project `jcamd`.

| Path | Role |
|------|------|
| `index.html` | Portfolio: flagship work, craft, lab, contact |
| `assets/portfolio.css` | Portfolio styles (dark theme) |
| `family/index.html` | Family Neural Architecture — write-up + client-side chart tool |
| `graphify/` | Static knowledge-graph viewer |

**Deploy:** push to `main` on `jcdavis131/jcamd`.

**Routes** (see `vercel.json`): `/arcade` redirects to hoops.dumbmodel.com;
`/knowledge` and `/journal` redirect to the home page.
